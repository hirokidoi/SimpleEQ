...
final class EQViewModelTests: XCTestCase {
    ...
    func testHandleAppliedSampleRateDidChangeKeepsLevelMeterTuningConsistentWithViewModel() {
        ...
        vm.handleAppliedSampleRateDidChange()
        audioWorld.queue.sync {}
    ...
    func testApplyPresetPersistsGainsPresetAndPreamp() {
        let store = SettingsStore(defaults: defaults)
        let engine = AudioEngine()
        let audioWorld = makeTestAudioWorld()
        let vm = EQViewModel(engine: engine, settings: store, outputController: makeOutputController(settings: store), audioWorld: audioWorld)
        ...
        let seed = EQSpec.builtInSeeds[.slot2]!
        XCTAssertEqual(store.gains, seed.curve)
        XCTAssertEqual(store.preset, .slot2)
        XCTAssertEqual(vm.selectedPreset, .slot2)
        XCTAssertEqual(vm.preampDb, seed.preamp)
        XCTAssertEqual(store.preampDb, seed.preamp)
        XCTAssertEqual(engine.preampGain, preampLinearGain(db: seed.preamp), accuracy: 0.0001)
    }

    // プリアンプがプリセット間で変わっても、適用した側の選択が残ること。
    func testApplyPresetKeepsSelectionEvenWhenPreampChangesFromThePreviousPreset() {
        let store = SettingsStore(defaults: defaults)
        let vm = makeVM(store)
        vm.applyPreset(.slot1)
        XCTAssertEqual(vm.selectedPreset, .slot1, "前提: 直前の選択が入っていること")
        XCTAssertNotEqual(
            EQSpec.builtInSeeds[.slot1]?.preamp, EQSpec.builtInSeeds[.slot2]?.preamp,
            "前提: 切り替え先のプリアンプが直前の値と異なること"
        )
    ...
    // プリアンプがプリセットの構成要素になった以上、プリアンプだけの変更も選択解除の対象になる。
    func testChangingPreampAloneClearsSelectedPreset() {
        let store = SettingsStore(defaults: defaults)
        let vm = makeVM(store)
        vm.applyPreset(.slot2)
        XCTAssertEqual(vm.selectedPreset, .slot2, "前提: プリセットが選択されていること")

        vm.preampDb = EQSpec.builtInSeeds[.slot2]!.preamp + 1

        XCTAssertNil(vm.selectedPreset)
    }

    // 整合判定は「解除」方向のみ。値を戻しても自動では選択が復帰しない。
    func testMovingPreampBackToThePresetValueDoesNotReselectIt() {
        let store = SettingsStore(defaults: defaults)
        let vm = makeVM(store)
        vm.applyPreset(.slot2)
        let seedPreamp = EQSpec.builtInSeeds[.slot2]!.preamp
        vm.preampDb = seedPreamp + 1
        XCTAssertNil(vm.selectedPreset, "前提: 一度解除されていること")

        vm.preampDb = seedPreamp

        XCTAssertNil(vm.selectedPreset)
    }

    // 起動時点でライブのプリアンプが保存プリセットと食い違っていれば、選択を持ち越さない。
    func testInitClearsSelectionWhenLivePreampDoesNotMatchSavedPreset() {
        let store = SettingsStore(defaults: defaults)
        store.preset = .slot2
        store.gains = EQSpec.builtInSeeds[.slot2]!.curve
        store.preampDb = EQSpec.builtInSeeds[.slot2]!.preamp + 1
    ...
    func testSavePresetOverwritesCurveTitlePreampAndSelectsPreset() {
        let store = SettingsStore(defaults: defaults)
        let vm = makeVM(store)
    ...
    func testResetPreampSetsZero() {
        ...
        vm.resetPreamp()
        XCTAssertEqual(vm.preampDb, 0)
    }

    func testPreampDragAndResetAreNoOpWhileBypassed() {
        let store = SettingsStore(defaults: defaults)
        let vm = makeVM(store)
        vm.preampDb = -4
        vm.bypass = true
        ...
        XCTAssertEqual(vm.preampDb, -4, "バイパス中はドラッグを無視すること")
        XCTAssertFalse(vm.draggingPreamp)

        vm.resetPreamp()
        XCTAssertEqual(vm.preampDb, -4, "バイパス中はリセットを無視すること")
    }
    ...
    func testProcessingSettingsRequestCarriesValueSnapshotAtSubmissionTime() {
        ...
        vm.preampDb = -3 // 依頼を投入 (キューは止まっているため、この時点ではまだ実行されない)。
    ...
    // handleDisplayGains と同型の追従。プレビュー中は動き続け、静定後に目標プリアンプとちょうど一致する。
    func testHandleDisplayPreampMovesDuringPresetPreviewThenSettlesExactlyAtTheValue() {
        let store = SettingsStore(defaults: defaults)
        let vm = makeVM(store)
        vm.confirmDriverProbe(.versionsUnreadable(.ok))
        let t0 = Date(timeIntervalSinceReferenceDate: 3000)
        let initialDisplayPreamp = vm.handleDisplayPreamp
        XCTAssertNotEqual(
            store.preamp(for: .slot2), initialDisplayPreamp, "前提: プレビュー先のプリアンプが初期値と異なること"
        )

        vm.previewPreset = .slot2
        vm.tick(now: t0)
        XCTAssertNotEqual(vm.handleDisplayPreamp, initialDisplayPreamp, "プレビュー値へ寄せ始めた最初の一歩で値が動くこと")

        for k in 1...300 { vm.tick(now: t0.addingTimeInterval(Double(k) * 0.016)) }
        XCTAssertEqual(vm.handleDisplayPreamp, store.preamp(for: .slot2), "静定の吸着により目標プリアンプへちょうど一致していること")
        vm.tick(now: t0.addingTimeInterval(301 * 0.016))
        XCTAssertEqual(vm.handleDisplayPreamp, store.preamp(for: .slot2), "静定後にさらに tick してもそこから動かないこと")
    }
    ...
    // EQ バンドのドラッグ中でも、プリアンプ側のプレビュー追従が止まらないこと。
    func testHandlePreampPreviewAdvancesWhileEqDragIsActive() {
        let store = SettingsStore(defaults: defaults)
        let vm = makeVM(store)
        vm.confirmDriverProbe(.versionsUnreadable(.ok))
        let t0 = Date(timeIntervalSinceReferenceDate: 5000)
        let initialDisplayPreamp = vm.handleDisplayPreamp
        XCTAssertNotEqual(store.preamp(for: .slot2), initialDisplayPreamp, "前提: プレビュー先のプリアンプが初期値と異なること")

        vm.previewPreset = .slot2
        vm.updateDrag(band: 0, db: 4)
        vm.tick(now: t0)

        XCTAssertNotEqual(vm.handleDisplayPreamp, initialDisplayPreamp, "EQ ドラッグ中でもプリアンプ側の追従が進むこと")
    }
