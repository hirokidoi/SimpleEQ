    ...
    func applyFrame(viewModel: EQViewModel, geo: EQPlotGeometry, meter: MeterChromeGeometry?, showLevelMeter: Bool, inEffect: Bool) {
        ...
        if !backChromeContainer.isHidden {
            ...
            if showLevelMeter, let meter {
                for layer in preampGainRangeLayers { layer.opacity = Float(handleAlpha) }
                applyMeterRange(
        ...
        if handleVisible {
            ...
            for band in 0..<EQSpec.bandCount {
                ...
                handleLineLayers[band].opacity = Float(handleAlpha)
            ...
            let preampLineVisible = showLevelMeter && meter != nil
            preampHandleLineLayer.isHidden = !preampLineVisible
            if preampLineVisible, let meter {
                preampHandleLineLayer.opacity = Float(handleAlpha)
