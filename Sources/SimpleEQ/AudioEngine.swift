...
final class AudioEngine: @unchecked Sendable {
    ...
    /// オーディオ世界のキュー上で発火する。
    var appliedSampleRateDidChange: (@Sendable () -> Void)?
    ...
    var makeEQUnit: (UInt32, @escaping AURenderCallback, UnsafeMutableRawPointer, AudioWorldToken) -> EQUnit? = { maxFrames, renderCallback, refCon, _ in
        ...
        guard eq.setup(maxFrames: maxFrames, renderCallback: renderCallback, refCon: refCon) else {
