...
final class SettingsStore {
    ...
    private static func normalized(_ decoded: Persisted) -> Persisted {
        ...
        p.preampAutoTargetDb = AutoPreampSpec.normalizedTargetDb(p.preampAutoTargetDb)
    ...
    func setGainsAndPreamp(_ gains: [Double], _ preampDb: Double) {
        writeState { $0.gains = gains; $0.preampDb = preampDb }
    }
    ...
    var preampAutoEnabled: Bool {
        get { readState { $0.preampAutoEnabled } }
        set { writeState { $0.preampAutoEnabled = newValue } }
    }

    var preampAutoTargetDb: Double {
        get { readState { $0.preampAutoTargetDb } }
        set { writeState { $0.preampAutoTargetDb = newValue } }
    }
    ...
    func deletePreset(_ preset: EQPreset) {
        writeState {
            $0.presetOverrides[preset] = PresetOverride(title: "", curve: Array(repeating: 0, count: EQSpec.bandCount))
        }
    }

    func savePreset(_ preset: EQPreset, curve: [Double], title: String) {
        writeState { $0.presetOverrides[preset] = PresetOverride(title: title, curve: curve) }
    }
}
