...
struct SettingsView: View {
    ...
    private var gainSection: some View {
        PanelSection("Gain") {
            preampSliderRow(
                ...
                ResetDotButton { viewModel.resetPreamp() }
            }
    ...
    private func preampSliderRow<Inline: View, Trailing: View>(
        ...
        sliderRow(
            title: title, subtitle: subtitle,
            value: preampSliderBinding(viewModel), range: PreampControl.range,
            format: { EQLayout.formatSignedDb($0) }, inline: inline, trailing: trailing
        )
    }

    private func sliderRow<Inline: View, Trailing: View>(
        title: String, subtitle: String? = nil,
