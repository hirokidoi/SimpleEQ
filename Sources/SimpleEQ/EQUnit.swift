...
final class EQUnit {
    ...
    private(set) var units: [AudioUnit] = []
    let bandCount: Int
    ...
    func setup(maxFrames: UInt32, renderCallback: @escaping AURenderCallback, refCon: UnsafeMutableRawPointer) -> Bool {
        ...
        guard setNonInterleavedStreamFormats() else {
    ...
    /// 全ユニットの input/output scope へ非interleaved のストリームフォーマットを設定する。
    /// 1 つでも拒否されたら false。
    private func setNonInterleavedStreamFormats() -> Bool {
        var asbd = AudioConfig.makeASBD()
        asbd.mFormatFlags |= kAudioFormatFlagIsNonInterleaved
        // 非interleaved は 1 AudioBuffer = 1ch ぶんのみを保持するため、フレームあたりの
        // バイト数は全ch分ではなく 1 サンプルぶんになる。
