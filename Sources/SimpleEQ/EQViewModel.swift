...
final class EQViewModel: ObservableObject {
    ...
    private(set) var gains: [Double] { didSet { displayRevision += 1 } }
    /// EQ バイパス (true = 素通し)。
    ...
    private func matchesCurrentState(_ preset: EQPreset) -> Bool {
        settings.curve(for: preset) == gains && settings.preamp(for: preset) == preampDb
    }
    ...
    func applyPreset(_ preset: EQPreset) {
        ...
        gains = target
        preampDb = settings.preamp(for: preset)
    ...
    func savePreset(_ preset: EQPreset, title: String) {
        ...
        settings.savePreset(preset, curve: gains, title: trimmedTitle, preamp: preampDb)
        settings.gains = gains
        selectedPreset = preset
        settings.preset = preset
        presetRevision += 1
    ...
    func tick(now: Date, processingInEffect: Bool? = nil) {
        ...
        updateHandleAlpha(dt: dt, processingInEffect: inEffect)
    }
    ...
    /// エンジン側設定を再適用する。
    func handleAppliedSampleRateDidChange() {
        applyProcessingSettingsToEngine()
    }
    ...
    private func advanceHandlePreampPreview(factor: Double) {
        ...
        let target = previewPreset.map { settings.preamp(for: $0) } ?? preampDb
    ...
    /// ハンドル表示アルファを更新する。表示条件を満たさなくなったら即座にフェードアウトを開始する。
    /// フェードはフレームレート非依存の時間基準 (dt から `1 - exp(-dt/tau)`)。
    private func updateHandleAlpha(dt: Double, processingInEffect: Bool) {
        let wantsVisible = processingInEffect && (handlesRevealed || hoveringPreset || savingPreset)
        let want: Double = wantsVisible ? 1 : 0
        guard want != handleAlpha else { return }

        let factor = 1 - exp(-dt / handleFadeTau)
        var next = handleAlpha + (want - handleAlpha) * factor
        if next < EQLayout.handleVisibilityThreshold {
            next = 0
        } else if 1 - next < EQLayout.handleVisibilityThreshold {
            next = 1
        }
