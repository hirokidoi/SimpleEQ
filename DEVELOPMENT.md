...
One surface inside the EQ window covers the visualizer outright, so what drives the contents underneath is gated on that surface being absent, and what drives the contents of the surface itself is gated on its being present and on those contents being the ones that carry a meter at all — both conditions carrying the window's own visibility as well. The two are derived at one place from those inputs rather than written from each side that shows or hides something, because a gate assembled from more than one input is where one of them gets left behind.
